...
boolean isEmptyList(struct PERSONA *testa) {
	if (testa == NULL)
		return true;
	else
		return false;
...
struct PERSONA *crea_lista() {
	struct PERSONA *testa = NULL; // il puntatore alla testa viene inizializzato a NULL
...
void ins_testa(INFO_FIELD dato, struct PERSONA **testa) {
	...
	struct PERSONA *nodo = calloc(1,sizeof(struct PERSONA)); // alloco dinamicamente lo spazio per un nuovo nodo
...
void elim_nodo(INFO_FIELD dato,struct PERSONA *testa) {
	...
	struct PERSONA *prec = NULL, *curr = testa;
...
void print_list(struct PERSONA *ptr) {
...
struct PERSONA *visit_list(struct PERSONA *testa) {
	struct PERSONA *prec = NULL, *curr = testa;
